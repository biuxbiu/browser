...
作为前端开发者，假如我们要做一个好的 `web` 应用或者优化已有的 `web` 应用，我们就需要对浏览器有一定的了解。了解浏览器的底层架构，对我们可以快速完成或者优化一个好的 `web` 应用起到了直观的重要。
...
>很久很久以前的浏览器很简单，它只是被用来单页面浏览网页。几乎没有什么很复杂的交互，动画，逻辑功能。那个时候，一个进程已经很满足当时的需求了。<Br>
时至今日，越来越多的 `web应用` ， 复杂的逻辑交互，炫酷的页面动画，一个进程已经不能满足于现状。多个进程来进行隔离，操作，运行，各个进程处理自己力所能及的事情，能让页面响应更快，用户体验更好。
