...
我们可以从控制台中看到页面资源的缓存来源：<br>

<img src="../img/cache.jpg">

有的时候是 `memory cache` ，有的时候是 `disk cache` ，如果你把控制台的 `Disabled cache` 选项打勾了的话，它会显示尺寸大小，这到底是怎么一回事呢？

那么服务器是怎么去执行缓存文件的呢？什么时候要用，什么时候不用，缓存文件是否会过期？

我们来看一个示意图：

<img src="../img/cache-process.jpg">

浏览器的缓存机制分为是否有缓存，再是缓存是否有过期，过期的话服务器的文件是否有改动？

* 第一步，判断该资源是否第一次访问，第一次访问的话会缓存该资源。如果不是第一次访问的话，会直接想服务器请求新文件。
* 第二步，判断该缓存文件是否已过期，没有的话直接使用缓存文件。有的话直接向服务器查找该文件，判断服务器该文件是否有修改过。
* 第三步，如果服务器该文件没有修改过，可能直接拉取服务器的文件，也可能直接使用本地的缓存。如果服务器该文件被修改过，那么服务器告知不能用之前的缓存文件了，会返回 `200` 并且带上最新的文件。

>从上面的例子图片可以看出，文件的状态码为 `200` ，但是会先从  `memory cache` 中得到，当我强制刷新的时候，文件的状态码依旧为 `200` ，但是是从 `disk cache` 中获取缓存文件。当我勾选了控制台的 `disabled cache` 之后，我们发现这个文件不使用本地缓存了，直接从服务器拉取。

<b>`memory cache` 和 `disk cache` 和 `资源本身的文件大小` 这个三种有什么区别？</b><Br>

`memory cache` ：从内存中获取。这个资源是直接从内存中获取到的，不会请求服务器；当这个页面关闭的话，这个资源就会被内存释放掉，再次打开就不会出现 `memory cache` 的情况；<br>
`disk cache` ：从本地磁盘中获取，即之前某个时间段已经加载过该资源，在本地磁盘存有副本，该资源不会随着页面关闭而消失。<br>
`资源本身大小` ：从服务器获取该资源。
